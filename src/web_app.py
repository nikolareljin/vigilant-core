...
from utils.gguf_summarizer import summarize_alerts_and_risk_snapshot  # noqa: E402
...
def api_insight() -> str:
    ...
    if not alerts:
        return jsonify({
            ...
            "risk_snapshot": "No risk snapshot available yet.",
    ...
    gguf_bundle = summarize_alerts_and_risk_snapshot(
        alerts=alerts[:30],
        subject=config.subject,
        location=config.location_name,
        question=config.question,
        enabled=bool(config.enable_gguf_summarizer),
        model_path=config.gguf_model_path,
        n_ctx=config.gguf_n_ctx,
        max_tokens=config.gguf_max_tokens,
    )
    ...
    try:
        ...
        if config.enable_gguf_summarizer:
            return jsonify({
                "has_question": True,
                "question": config.question,
                "summary": gguf_bundle.alert_summary,
                "explanation": "Generated by local GGUF summarizer because Ollama is unavailable.",
                "risk_snapshot": gguf_bundle.risk_snapshot,
                "suggestions": [],
                "suggestions_origin": "none",
                "sources_used": list(sources_used)[:10],
                "summary_engine": gguf_bundle.engine,
                "summary_error": gguf_bundle.error,
            })
        return jsonify({
            "has_question": True,
            "question": config.question,
    ...
    try:
        ...
        return jsonify({
            ...
            "risk_snapshot": gguf_bundle.risk_snapshot if config.enable_gguf_summarizer else "",
            "suggestions": suggestions,
            "suggestions_origin": _normalize_suggestions_origin(payload.get("suggestions_origin"), suggestions),
            "sources_used": list(sources_used)[:10],
            "summary_engine": "ollama",
            "summary_error": gguf_bundle.error if config.enable_gguf_summarizer else None,
        })
...
def api_risk_snapshot() -> str:
    """Return a GGUF/heuristic local risk snapshot built from recent alerts."""
    config = get_config()
    alerts = database.fetch_recent(50)
    ...
    return jsonify(
        {
            "subject": config.subject,
            "location": config.location_name,
            "risk_snapshot": bundle.risk_snapshot,
            "alert_summary": bundle.alert_summary,
            "summary_engine": bundle.engine,
            "summary_error": bundle.error,
            "alerts_considered": min(30, len(alerts)),
        }
    )
